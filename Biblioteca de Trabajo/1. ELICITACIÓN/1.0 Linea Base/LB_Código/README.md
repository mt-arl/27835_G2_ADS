...
Línea Base - Código (Versión 2.0)
Aquí se encuentra todo el código fuente y scripts del proyecto KairosMix. Esta versión marca la evolución hacia un Backend seguro con implementación de autenticación y arquitectura escalable.
...
npm run lint: Análisis de código
Patrones de Diseño Implementados:
Singleton: Gestión única de conexión a MongoDB (Database.ts).
Factory: Creación dinámica de repositorios (RepositoryFactory) para desacoplar la lógica de la base de datos.
Repository: Abstracción del acceso a datos (CRUDs separados de controladores).
Middleware: Interceptores para validar seguridad (JWT) antes de procesar peticiones.
