...
/**
 * COMPARATIVA: SINGLETON vs BUILDER
 *
 * Pruebas exhaustivas que comparan ambos patrones de diseño
 */
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
public class SingletonVsBuilderTest {

    @BeforeAll
    public static void setup() {
        System.out.println("\n╔════════════════════════════════════════════════════════════════╗");
        System.out.println("║     PRUEBAS COMPARATIVAS: SINGLETON VS BUILDER PATTERN       ║");
        System.out.println("╚════════════════════════════════════════════════════════════════╝\n");

        // Activar modo test para usar cédulas de prueba
        EstudianteBuilder.activarModoTest();
    }
    ...
    // ============================================================================
    // MÉTRICA 2: BUILDER - MÚLTIPLES INSTANCIAS INDEPENDIENTES
    // ============================================================================

    @Test
    @Order(4)
    @DisplayName("BUILDER - Crea múltiples instancias DIFERENTES")
    public void test04_BuilderMultiplesInstancias() throws Exception {
        System.out.println("\n MÉTRICA 4: Builder crea instancias independientes");
        System.out.println("─────────────────────────────────────────────────────────────");

        Estudiante est1 = EstudianteBuilder.crearEstudiante("1234567890", "Ana Torres", "20");
        Estudiante est2 = EstudianteBuilder.crearEstudiante("0987654321", "Pedro Sánchez", "28");
        Estudiante est3 = EstudianteBuilder.crearEstudiante("1111111111", "Lucía Mendoza", "35");
    ...
    @Test
    @Order(5)
    @DisplayName(" PERFORMANCE - Singleton vs Builder (1000 operaciones)")
    public void test05_PerformanceComparativo() throws Exception {
        ...
        for (int i = 0; i < 1000; i++) {
            Estudiante est = EstudianteBuilder.crearEstudiante(
                cedulasValidas[i % 10], 
                "Test" + i, 
        ...
        long duracionBuilder = (finBuilder - inicioBuilder) / 1_000_000;

        System.out.println("  SINGLETON (1000 accesos):  " + duracionSingleton + " ms");
        System.out.println("️  BUILDER (1000 creaciones): " + duracionBuilder + " ms");
        System.out.println(" Factor de diferencia: " + (duracionBuilder / Math.max(duracionSingleton, 1)) + "x");
        System.out.println(" CONCLUSIÓN: Singleton es MÁS RÁPIDO para acceso\n");

        assertTrue(duracionSingleton < duracionBuilder, 
                   "Singleton debe ser más rápido que Builder");
    }
    ...
    @Test
    @Order(6)
    @DisplayName(" MEMORIA - Singleton vs Builder (1000 objetos)")
    public void test06_ConsumoMemoria() throws Exception {
        ...
        long memoriaBuilder = runtime.totalMemory() - runtime.freeMemory();
        long usoBuilder = memoriaBuilder - memoriaSingleton;

        System.out.println(" SINGLETON (1 instancia):     ~" + (usoSingleton / 1024) + " KB");
        System.out.println(" BUILDER (1000 instancias):   ~" + (usoBuilder / 1024) + " KB");
    ...
    public void test07_RendimientoBajoCarga() throws Exception {
        ...
        // Test Builder bajo carga
        String[] cedulas = {"1234567890", "0987654321", "1111111111", "0123456789", "0606060606",
                            "1010101010", "1313131313", "1414141414", "0105050505", "1717171717"};
        long inicioBuilder = System.nanoTime();
        for (int i = 0; i < operaciones; i++) {
            EstudianteBuilder.crearEstudiante(
                cedulas[i % 10],
                "Nombre" + i,
                String.valueOf(18 + (i % 50))
            );
        }
        long tiempoBuilder = (System.nanoTime() - inicioBuilder) / 1_000_000;

        double mejora = (double) tiempoBuilder / Math.max(tiempoSingleton, 1);

        System.out.println("️  SINGLETON (10K accesos):    " + tiempoSingleton + " ms");
        System.out.println("️  BUILDER (10K creaciones):   " + tiempoBuilder + " ms");
        System.out.println(" Diferencia: " + String.format("%.2f", mejora) + "x más lento Builder");
        System.out.println(" Throughput Singleton: " + String.format("%,d", operaciones * 1000 / Math.max(tiempoSingleton, 1)) + " ops/seg");
        System.out.println(" Throughput Builder:   " + String.format("%,d", operaciones * 1000 / Math.max(tiempoBuilder, 1)) + " ops/seg");
    ...
    @Test
    @Order(8)
    @DisplayName(" TESTABILIDAD - Builder es más testeable")
    public void test08_Testabilidad() {
        System.out.println("\n MÉTRICA 8: Facilidad de Testing");
        System.out.println("─────────────────────────────────────────────────────────────");

        // BUILDER - Testeable sin dependencias
        assertDoesNotThrow(() -> {
            Estudiante est = EstudianteBuilder.crearEstudiante("1234567890", "Test", "25");
            assertNotNull(est);
            assertEquals("1234567890", est.getId());
            assertEquals("Test", est.getNombres());
            assertEquals(25, est.getEdad());
        });
        System.out.println(" Builder: Testeable SIN mocks ni dependencias");
        ...
        System.out.println(" CONCLUSIÓN: Builder tiene MAYOR testabilidad\n");
    }
    ...
    public void test09_GarbageCollectionImpact() throws Exception {
        ...
        // Test Builder - Genera muchos objetos temporales
        runtime.gc();
        Thread.sleep(100);
        long gcAntes = 0;
        ...
        for (int i = 0; i < 5000; i++) {
            EstudianteBuilder.crearEstudiante(cedulas[i % 5], "Test" + i, "25");
        }
        ...
        System.out.println("  Builder (5000 objetos): " + coleccionesGC + " ciclos de GC");
        System.out.println(" Singleton (1 objeto):    0 ciclos de GC adicionales");
        System.out.println(" CONCLUSIÓN: Builder genera más presión en GC\n");
    }
    ...
    public void test10_AnalisisLatencia() throws Exception {
        ...
        long[] latenciasBuilder = new long[muestras];
        ...
        // Medir latencias Builder
        String[] cedulas = {"1234567890", "0987654321", "1111111111"};
        for (int i = 0; i < muestras; i++) {
            long inicio = System.nanoTime();
            EstudianteBuilder.crearEstudiante(cedulas[i % 3], "Test" + i, "25");
            latenciasBuilder[i] = System.nanoTime() - inicio;
        }

        // Calcular percentiles
        java.util.Arrays.sort(latenciasSingleton);
        java.util.Arrays.sort(latenciasBuilder);
        ...
        long p50Builder = latenciasBuilder[muestras / 2] / 1000;
        long p95Builder = latenciasBuilder[(int)(muestras * 0.95)] / 1000;
        long p99Builder = latenciasBuilder[(int)(muestras * 0.99)] / 1000;
        ...
        System.out.println("\n BUILDER:");
        System.out.println("   • P50 (mediana): " + p50Builder + " μs");
        System.out.println("   • P95:           " + p95Builder + " μs");
        System.out.println("   • P99:           " + p99Builder + " μs");
    ...
    public void test11_Concurrencia() throws Exception {
        ...
        // Test Builder en concurrencia
        Thread[] threadsBuilder = new Thread[numThreads];
        long inicioBuilder = System.nanoTime();

        String[] cedulas = {"1234567890", "0987654321", "1111111111"};
        for (int i = 0; i < numThreads; i++) {
            final int threadId = i;
            threadsBuilder[i] = new Thread(() -> {
                try {
                    for (int j = 0; j < operacionesPorThread; j++) {
                        EstudianteBuilder.crearEstudiante(
                            cedulas[j % 3],
            ...
            threadsBuilder[i].start();
        }

        for (Thread t : threadsBuilder) {
            t.join();
        }
        long tiempoBuilder = (System.nanoTime() - inicioBuilder) / 1_000_000;

        System.out.println(" SINGLETON (10 threads, 100 ops c/u): " + tiempoSingleton + " ms");
        System.out.println(" BUILDER (10 threads, 100 ops c/u):   " + tiempoBuilder + " ms");
        System.out.println(" Singleton usa synchronized (serialización)");
        System.out.println(" Builder es naturalmente thread-safe");
        System.out.println(" CONCLUSIÓN: Builder mejor para escenarios concurrentes\n");
    }
    ...
    @Test
    @Order(12)
    @DisplayName("🔗 ACOPLAMIENTO - Builder tiene bajo acoplamiento")
    ...
    public void test13_TablaComparativa() {
        ...
        System.out.println("╠════════════════════════════════════════════════════════════════════════╣");
        System.out.println("║ MÉTRICA                     │ SINGLETON          │ BUILDER              ║");
        System.out.println("╠════════════════════════════════════════════════════════════════════════╣");
        ...
        System.out.println("║  6. Concurrencia multi-thread       - Builder gana                    ║");
        System.out.println("║  7. Testabilidad                    - Builder gana                    ║");
        System.out.println("║  8. Acoplamiento/Cohesión           - Builder gana                    ║");
        System.out.println("╠════════════════════════════════════════════════════════════════════════╣");
        ...
        System.out.println("║                                                                        ║");
        System.out.println("║ • USAR BUILDER cuando:                                                ║");
        ...
        System.out.println("║   → Builder para creación/validación de objetos                       ║");
        System.out.println("╚════════════════════════════════════════════════════════════════════════╝\n");
    ...
    public static void cleanup() {
        ...
        EstudianteBuilder.desactivarModoTest();
